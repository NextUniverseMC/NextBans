    ...
    public void execute(CommandSender commandSender, String[] strings) {
        if (strings.length >= 3) {
            ...
            if (strings[0].length() > 16 && strings[0].length() != 36) {
                commandSender.sendMessage(new ComponentBuilder("Invalid arguments! ").color(ChatColor.RED).append("/tempmute <username or UUID> <time> <reason>").color(ChatColor.WHITE).create());
            }
                ...
                try (Connection connection = Main.getHikari().getConnection();
                    ...
                    final String innerUUID = uuid;
                        ...
                        try (Connection connection2 = Main.getHikari().getConnection();
                            ...
                            if (result2.first()) {
                                commandSender.sendMessage(new ComponentBuilder("That player is already muted! Use ").color(ChatColor.RED).append("/unmute <username>").color(ChatColor.WHITE).append(" to unmute them first.").color(ChatColor.RED).create());
                            }
                            else {
                                    ...
                                    public void run() {
                                        try (Connection connection = Main.getHikari().getConnection();
                                            ...
                                            try {
                                                ...
                                                if (p.isConnected()) {
                                                    String relTime = DateFormatter.format(time);
                                                    CenterText.sendCenteredMessage(p, ChatColor.DARK_RED + "=====================================================");
                                                    CenterText.sendCenteredMessage(p, ChatColor.RED + "" + ChatColor.BOLD + "You are temporarily muted for " + ChatColor.WHITE + "" + ChatColor.BOLD +
                                                            relTime);
                                            ...
                                            for (ProxiedPlayer a : ProxyServer.getInstance().getPlayers()) {
                                                if (a.hasPermission("bans.broadcast")) {
                                                    a.sendMessage(Main.getMessage(reason, time, Main.ReasonType.TEMPMUTE_BROADCAST, commandSender.getName(), name, rawDate));
                                                }
                                            }
                                        }
                        ...
                        ProxyServer.getInstance().getScheduler().runAsync(Main.getInstance(), new Runnable() {
                            ...
                            public void run() {
                                try (Connection connection = Main.getHikari().getConnection();
                                    ...
                                    for (ProxiedPlayer a : ProxyServer.getInstance().getPlayers()) {
                                        if (a.hasPermission("bans.broadcast")) {
                                            a.sendMessage(Main.getMessage(reason, time, Main.ReasonType.TEMPMUTE_BROADCAST, commandSender.getName(), uuid, rawDate));
                                        }
                                    }
                                } catch (SQLException e) {
                                    e.printStackTrace();
